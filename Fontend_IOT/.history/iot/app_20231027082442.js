...
function warning() {
  if (countLight > 800) {
    // Nếu countLight lớn hơn 800, thêm lớp CSS "warning" vào phần tử cần áp dụng
    document.getElementByClass('block3 block-size').classList.add('warning');
  } else {
    // Nếu countLight không lớn hơn 800, loại bỏ lớp CSS "warning" khỏi phần tử cần áp dụng (nếu đã có)
    document.getElementByClass('block3 block-size').classList.remove('warning');
  }
}
