...
function createPageSelect(totalPages, currentPage, ArrayData) {
  ...
  pageSelect.addEventListener("change", (event) => { // Sự kiện khi chọn trang
    const selectedPage = parseInt(event.target.value, 10);  // lấy giá trị của phần tử <option> được chọn
...
getAllData().then(() => { // Lấy dữ liệu từ API và hiển thị nó lên bảng
  ...
  createPageSelect(totalPages, currentPage, dataFromSpringBoot);// Tạo các option trang
  ...
  function search() {
    const searchText = searchInput.value.trim().toLowerCase(); // ô nhập dữ liệu được lấy thông qua searchInput.value
    const selectedColumn = searchColumn.value; // cột dữ liệu cần tìm kiếm được lấy thông qua searchColumn.value
