...
let onLed2Count = 0;
let offLed2Count = 0;
function controlLed(ledCommand, onCallback, offCallback, onCountElement, offCountElement) {
    ...
    fetch("http://localhost:8080/led", requestOptions)
        .then(data => {
            ...
            // Update count elements accordingly
            document.getElementById(onCountElement).innerText = onLed1Count;
            document.getElementById(offCountElement).innerText = offLed1Count;
            document.getElementById(onCountElement).innerText = onLed2Count;
            document.getElementById(offCountElement).innerText = offLed2Count;
        })
        .catch(error => {
...
document.querySelector(".onLed1").addEventListener("click", function() {
    controlLed("onled1", OnFan, OffFan, "fanCount", "offfanCount");
});

document.querySelector(".offLed1").addEventListener("click", function() {
    controlLed("offled1", OnFan, OffFan, "fanCount", "offfanCount");
});

document.querySelector(".onLed2").addEventListener("click", function() {
    controlLed("onled2", OnLight, OffLight, "lightCount", "offlightCount");
});

document.querySelector(".offLed2").addEventListener("click", function() {
    controlLed("offled2", OnLight, OffLight, "lightCount", "offlightCount");
});

// Display initial values
document.getElementById("fanCount").innerText = onLed1Count;
document.getElementById("offfanCount").innerText = offLed1Count;
document.getElementById("lightCount").innerText = onLed2Count;
document.getElementById("offlightCount").innerText = offLed2Count;
