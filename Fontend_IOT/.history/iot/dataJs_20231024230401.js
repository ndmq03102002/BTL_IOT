let dataFromSpringBoot = []
   function getTempAllData(){
        ...
        return fetch(`http://localhost:8080/getAllData`)
...
function displayData(data) {
    ...
    data.forEach(rowData => {
        ...
        row.innerHTML = `<td>${rowData.id}</td><td>${rowData.temperature}</td><td>${rowData.humidity}</td><td>${rowData.light}</td><td>${rowData.mua}</td><td>${rowData.time}</td></td>`;
    ...
    const selectedColumn = searchColumn.value;
     ...
     function Search(){
        ...
        dataSearch= dataFromSpringBoot.filter(a =>{
            if(selectedColumn=="temperature"){
                if(a.temperature==searchText){
                    return true;
                }
            }
            else if(selectedColumn=="humidity"){
                if(a.humidity==searchText){
                    return true;
                }
            }
...
///--------------------------------------Sort-----------------------------------------------------------------------------------
        ...
        button_sort_temp_incre = document.querySelector("#sort-by-temperature-incre").addEventListener('click',()=>{
            DataSort = SortIncrease("temperature")
        ...
        button_sort_hum_incre = document.querySelector("#sort-by-humidity-incre").addEventListener('click',()=>{
            DataSort = SortIncrease("humidity")
            let totalPages = Math.ceil(DataSort.length/10); 
            let currentPage = 1;
            displayPage(currentPage,DataSort)
            updatePaginationUI(totalPages,currentPage,DataSort);
        ...
        button_sort_light_incre = document.querySelector("#sort-by-mua-incre").addEventListener('click',()=>{
            DataSort = SortIncrease("mua")
            let totalPages = Math.ceil(DataSort.length/10); 
            let currentPage = 1;
            displayPage(currentPage,DataSort)
            updatePaginationUI(totalPages,currentPage,DataSort);
        ...
        button_sort_temp_incre = document.querySelector("#sort-by-temperature-decre").addEventListener('click',()=>{
            DataSort = SortDecrease("temperature")
            let totalPages = Math.ceil(DataSort.length/10); 
            let currentPage = 1;
            displayPage(currentPage,DataSort)
            updatePaginationUI(totalPages,currentPage,DataSort);
        ...
        button_sort_hum_incre = document.querySelector("#sort-by-humidity-decre").addEventListener('click',()=>{
            DataSort = SortDecrease("humidity")
            let totalPages = Math.ceil(DataSort.length/10); 
            let currentPage = 1;
            displayPage(currentPage,DataSort)
            updatePaginationUI(totalPages,currentPage,DataSort);
        ...
        button_sort_light_incre = document.querySelector("#sort-by-mua-decre").addEventListener('click',()=>{
            DataSort = SortDecrease("mua")
            let totalPages = Math.ceil(DataSort.length/10); 
            let currentPage = 1;
            displayPage(currentPage,DataSort)
            updatePaginationUI(totalPages,currentPage,DataSort);
